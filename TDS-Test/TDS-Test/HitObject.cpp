...
// Hit poly;
HitPoly::HitPoly() {}
HitPoly::HitPoly(const HitPoly &hitPoly, GLfloat angle) : vertices(hitPoly.GetVertices()), axes(hitPoly.GetAxes) { this->RotatePoly(angle); }
...
HitPoly::HitPoly(const HitPoly &rHitPoly, glm::vec2 size, GLfloat angle) : HitPoly(rHitPoly, size) { this->RotatePoly(angle); }

void HitPoly::UpdateAxes() 
{
...
void HitPoly::RotatePoly(GLfloat angle)
{
...
// Hitbox
HitBox::HitBox(glm::vec2 pos, glm::vec2 size, GLfloat angle) : pos(pos), size(size), angle(angle) { this->axes.reserve(2); this->vertices.reserve(4); }
HitBox::HitBox(const HitBox& hitBox, glm::vec2 pos, GLfloat angle) : pos(hitBox.GetPos() + pos), angle(hitBox.GetAngle() + angle), size(hitBox.GetSize()) { this->UpdateHbox(); }
HitBox::HitBox(const HitBox& hitBox, glm::vec2 pos, GLfloat angle) : pos(hitBox.GetPos() + pos), angle(hitBox.GetAngle() + angle), size(hitBox.GetSize() * size) { this->UpdateHbox(); }
HitBox::HitBox(const HitBox& rHitBox, glm::vec2 size) : HitPoly(rHitBox) { this->Scale(size * 0.5f); }


void HitBox::UpdateHbox()
{
...
void HitBox::RotatePoly(GLfloat angle)
{
	glm::mat2 rotMat = Util::RotationMat2(angle);
	for (glm::vec2 &v : this->vertices) { v = rotMat * v; }
	for (glm::vec2 &a : this->axes) { a = rotMat * a; }
...
// Getters and Setters
glm::vec2 HitBox::GetPos() const { return this->pos; }
