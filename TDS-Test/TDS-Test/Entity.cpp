...
Entity::Entity(const Etex *etex, GLfloat width, GLfloat height) : pos(glm::vec2(0), 1.0f), angle(0), tex(etex->GetTex()), hitObjs(etex->GetHitComb(width)), size(etex->GetTexSize(width), height), color(glm::vec3(1)) {}
Entity::Entity(const Etex* etex, glm::vec3 size) : pos(glm::vec2(0), 1.0f), angle(0), tex(etex->GetTex()), hitObjs(etex->GetHitComb(size)), size(size), color(glm::vec3(1)) {}
