#include "LevelTest.h"
#include "..\Entities\Enemies\E_Drone.h"
#include "..\Entities\Enemies\E_MotherDrone.h"
#include "..\Entities\StaticEntity.h"
#include "..\Entities\Enemies\E_Jelly.h"
#include "..\Entities\Enemies\E_ArtilBot.h"
#include "..\Entities\Players\Ranger\P_Ranger.h"
...
void LevelTest::Reset() {
	...
	for (GLuint i = 0; i < Util::PLAYER_AMOUNT; i++) {
		Game::sPlayers.push_back(new P_Ranger(glm::vec2(0, 3 * i)));
...
void LevelTest::spawnNextWave() {
	...
	for (GLuint i = 0; i < WAVES[wavecounter][DRONE]; i++) {
		Game::sEnemies.push_back(new E_Drone(wave_SpawnPos));
	}
	for (GLuint i = 0; i < WAVES[wavecounter][MOTHER_DRONE]; i++) {
		Game::sEnemies.push_back(new E_MotherDrone(wave_SpawnPos));
	}
	for (GLuint i = 0; i < WAVES[wavecounter][SLIME]; i++) {
