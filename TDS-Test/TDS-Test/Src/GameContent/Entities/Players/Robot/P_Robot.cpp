...
Robot::Robot(glm::vec2 position) : Player(position)
{
	...
	subEntities["tracks"] = new SE_BodyPart(this, glm::vec2(0), &ResourceManager::GetEtex("D_Bot"), 1.3f, 1);
	...
	subEntities["tracks"]->SetTurnSpeed(5);
...
GLboolean Robot::UpdateE(GLfloat dt) {
	...
	if (artilMode) { // Artilelry Mode
		...
		case SPRINTING:
			...
			accuracy = 0.1f * (1.0f + 3.0f / Util::CONTROLLER_TRIGGER_MAX * (GLfloat)gPad.bLeftTrigger);
			break;
		}
		if (gPad.wButtons & XINPUT_GAMEPAD_RIGHT_SHOULDER) {
			shoot();
		...
		subEntities["tracks"]->LookAt(movDir, dt);
	}
