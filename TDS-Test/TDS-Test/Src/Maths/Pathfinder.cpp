...
void Pathfinder::MakePolyConvex(HitPoly &poly)
{
	...
	do {
		...
		if (index0 == (GLint) vertices.size()) { index0 = 0; }
		if (index1 == (GLint) vertices.size()) { index1 = 0; }
		if (index2 == (GLint) vertices.size()) { index2 = 0; }
		if (index0 < 0) { index0 = (GLint) vertices.size() - 1; }
		if (index1 < 0) { index1 = (GLint) vertices.size() - 1; }
		if (index2 < 0) { index2 = (GLint) vertices.size() - 1; }
	} while (index1 != 0);
...
GLboolean Pathfinder::CheckSinglePoints(std::vector<SinglePoint> &singlePoints, const HitPoly &hP)
{
	...
	for (glm::vec2 axis : hP.GetAxes()) {
		...
		tVertices = hP.GetVertices();
		tVerticesSize = (GLuint) tVertices.size();

		tPolyProjMax = tPolyProjMin = glm::dot(tVertices.front(), axis);
		for (GLint i = 1; i < (GLint) tVerticesSize; i++) {
		...
		for (GLint i = 0; i < (GLint) singlePoints.size(); i++) {
