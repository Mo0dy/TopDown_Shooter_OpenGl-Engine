...
GLboolean E_Jelly::PreUpdate() {
	...
	if (this->death) {
		if (jellySize > 0.7)
		{
			Game::sSpawnE.push_back(new E_Jelly(this->Get2DPos() + glm::vec2(1),  this->jellySize / (GLfloat) glm::sqrt(2)));
			Game::sSpawnE.push_back(new E_Jelly(this->Get2DPos() + glm::vec2(1.5), this->jellySize / (GLfloat) glm::sqrt(2)));
		}
