...
Entity::Entity(const Etex *etex, GLfloat width, GLfloat height) : pos(glm::vec2(0), 1.0f), angle(0), tex(etex->GetTex()), hitObjs(etex->GetHitObjs(width)), size(etex->GetTexSize(width), height), color(glm::vec3(1)) {}
Entity::Entity(const Etex* etex, glm::vec3 size) : pos(glm::vec2(0), 1.0f), angle(0), tex(etex->GetTex()), hitObjs(etex->GetHitObjs(size)), size(size), color(glm::vec3(1)) {}
...
void Entity::UpdateAni() { animations[ani].UpdateAni(this); }
...
void Entity::Set2DSize(glm::vec2 size) { this->size = glm::vec3(size, this->size.z); }
void Entity::Set2DSize(GLfloat width) { this->size = glm::vec3(width, (GLfloat) this->tex->Height / this->tex->Width * width, this->size.z); }
